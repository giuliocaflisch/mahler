...
@[simp] theorem fwdDiff_zero_iterate (h : M) (n : ℕ) :
  ...
  · simp_rw [Function.iterate_zero, id_eq]
  · simp_rw [Function.iterate_succ_apply', hn, fwdDiff_const]
...
theorem fwdDiff_iter_eq_sum_shift (h : M) (n : ℕ) (f : M → G) (x : M) :
  ...
  revert x
  induction' n with n hn
  · simp_rw [Function.iterate_zero, id_eq, zero_add, Finset.range_one, Finset.sum_singleton,
      Nat.sub_zero, pow_zero, Nat.choose_zero_right, Nat.cast_one, mul_one, one_smul, zero_smul, add_zero, implies_true]
  · intro z
...
theorem IsUltrametricDist.norm_fwdDiff_iter_apply_le {M G : Type*} [TopologicalSpace M] [CompactSpace M] [AddCommMonoid M] [SeminormedAddCommGroup G] [IsUltrametricDist G]
  ...
  · simp only [Finset.nonempty_range_iff, ne_eq, AddLeftCancelMonoid.add_eq_zero, one_ne_zero,
